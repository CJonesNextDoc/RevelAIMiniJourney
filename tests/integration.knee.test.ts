...
test('integration: knee trigger follows false branch and completes', async () => {
  const journeyPath = path.resolve(process.cwd(), 'examples', 'journeys', 'example_journey.json');
  const triggerPath = path.resolve(process.cwd(), 'examples', 'triggers', 'example_trigger_knee.json');
  ...
  // condition should have evaluated to false and the KOOS message should have been sent
  const hasCondition = steps.some((s: any) => s.type === 'condition_evaluated' && s.payload && s.payload.result === false);
